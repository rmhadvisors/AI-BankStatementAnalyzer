...
export function scopeReportByMonth(report: AnalysisReport, monthKey: string): AnalysisReport {
  ...
  return {
    ...
    metrics: months,
    momSummary: months,
    camAnalysis: filterMonthRows(report.camAnalysis, monthKey, monthLabel),
    monthlyCF: filterMonthRows(report.monthlyCF, monthKey, monthLabel),
    netTransactions: filterMonthRows(report.netTransactions, monthKey, monthLabel),
    staffEmoluments: filterMonthRows(report.staffEmoluments, monthKey, monthLabel),
    salary: (report.salary || []).filter(
      (row) => row.monthKey === monthKey || row.month === monthLabel,
    ),
    bounces: filterMonthRows(report.bounces, monthKey, monthLabel),
    emiTracker: filterMonthRows(report.emiTracker, monthKey, monthLabel),
    ...
    highestTns: filterMonthRows(report.highestTns, monthKey, monthLabel),
    internalGroup: filterMonthRows(report.internalGroup, monthKey, monthLabel),
