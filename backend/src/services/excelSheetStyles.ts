...
function isSectionRow(first: string): boolean {
  return SECTION_TITLE_LABELS.has(first);
}
...
function styleNavRow(row: ExcelJS.Row): void {
  row.eachCell({ includeEmpty: true }, (cell) => {
    const text = cellText(cell.value);
    if (text === "Index" || text === "Go to top") {
      cell.font = { name: "Calibri", bold: true, underline: true, color: { argb: EXCEL_THEME.link } };
    } else if (row.number === 1 && cell.address?.startsWith("A")) {
      cell.font = { name: "Calibri", bold: true, size: 12, color: { argb: EXCEL_THEME.primary } };
    } else if (row.number === 2 && cell.address?.startsWith("A")) {
